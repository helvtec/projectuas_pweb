...
if($judul == "" || $penerbit == "" || $pengarang== "" || $tahun == "" || $harga== "" || $stok== "" ){
    echo"<script>
        ...
        window.location = 'edit_buku.php?id=$id';
    </script>";
} else {
    ...
    if ($gambar != "") {
        ...
        if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
            $uploaded = "C:/xampp/htdocs/projectuas_pweb/gambar/buku/";
            move_uploaded_file($file_tmp, $uploaded.$nama_gambar_baru);
            // jalankan query INSERT untuk menambah data ke database pastikan sesuai urutan (id tidak perlu karena dibikin otomatis)
            $query = "UPDATE buku SET judul='$judul', penerbit='$penerbit', pengarang='$pengarang', tahun_terbit='$tahun', harga='$harga', stok='$stok', gambar='$nama_gambar_baru' WHERE ID = $id";
            // $query.= "WHERE ID = $id";
